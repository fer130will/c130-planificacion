<?php
$conexion1 = pg_connect("host=localhost dbname=datosAviones893 user=postgres password=1978Wilson");
$query = "SELECT numvuelos,hora,segundo,alttotal,altprom,cargtotal,cargprom,usabilidad FROM cortol";
$consulta8931 = pg_query($conexion1, $query);
for ($i = 0; $i < pg_num_rows($consulta8931); $i++) {
    if ($obj1 = pg_fetch_object($consulta8931)) {
    }
}
pg_close();
//----------------Corto end -----------------------------------//
...
$query = "SELECT numvuelos,hora,segundo,alttotal,altprom,cargtotal,cargprom,usabilidad FROM largol";
$consulta8932 = pg_query($conexion2, $query);
for ($i = 0; $i < pg_num_rows($consulta8932); $i++) {
    if ($obj2 = pg_fetch_object($consulta8932)) {
    }
}
pg_close();
//---------- Largo end ---------------------------------------//
//---------- tactico ---------------------------------------//
$conexion3 = pg_connect("host=localhost dbname=datosAviones893 user=postgres password=1978Wilson");
$query = "SELECT numvuelos,hora,segundo,alttotal,altprom,cargtotal,cargprom,usabilidad FROM tacticol";
$consulta8933 = pg_query($conexion3, $query);
for ($i = 0; $i < pg_num_rows($consulta8933); $i++) {
    if ($obj3 = pg_fetch_object($consulta8933)) {
    }
}
pg_close();
//---------- Tactico end ---------------------------------------//
//---------- Entrenamiento ---------------------------------------//
$conexion4 = pg_connect("host=localhost dbname=datosAviones893 user=postgres password=1978Wilson");
$query = "SELECT numvuelos,hora,segundo,alttotal,altprom,cargtotal,cargprom,usabilidad FROM trainingl";
$consulta8934 = pg_query($conexion4, $query);
for ($i = 0; $i < pg_num_rows($consulta8934); $i++) {
    if ($obj4 = pg_fetch_object($consulta8934)) {
    }
}
pg_close();
//---------- Entrenamiento end ---------------------------------------//
//---------- factor de severidad ---------------------------------------//
$conexion5 = pg_connect("host=localhost dbname=datosAviones893 user=postgres password=1978Wilson");
$query = "SELECT totalhoras,fscorto,fslargo,fsentre,fstactico,fstotal FROM fseveridadl";
$consulta8935 = pg_query($conexion5, $query);
for ($i = 0; $i < pg_num_rows($consulta8935); $i++) {
    if ($obj5 = pg_fetch_object($consulta8935)) {
    }
}
pg_close();
//---------- factor de severidad end ---------------------------------------//
//---------- EFH ---------------------------------------//
$conexion6 = pg_connect("host=localhost dbname=datosAviones893 user=postgres password=1978Wilson");
$query = "SELECT cefh,lefh,eefh,tefh,total FROM efhl";
$consulta8936 = pg_query($conexion6, $query);
for ($i = 0; $i < pg_num_rows($consulta8936); $i++) {
    if ($obj6 = pg_fetch_object($consulta8936)) {
    }
}
pg_close();
//---------- EFH end ---------------------------------------//
...
<body>
    ...
    <div class="jumbotron jumbotron-fluid bg-jumbotron" style="margin-bottom: 20px;">
        <div class="container text-center py-5">
            <h3 class="text-white display-3 mb-4">FAE 893</h3>
            <h6 class="text-white">Ingrese los datos de acuerdo a las aeronaves</h6>
    ...
    <!-- datos inicio------------------------------------------------------------------->
    <form action="conexion893.php" method="post" class="formulario-datos">
